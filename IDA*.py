...
def get_locations(loc_filepath):
    '''Read the file and return a list of all locations
    '''
    with open(loc_filepath) as f:
        lines = [line.rstrip() for line in f]
    return lines

def get_distance_matrix(dist_filepath):
    ...
    with open(dist_filepath) as csvfile:
...
class IDASearchEngine:
    ...
    def search(self, initState, goal_fn, ss, heur_fn=_zero_hfn, LIMIT=1):
        ...
        #hval = heur_fn(initState)
        first_node = sNode(initState, heur_fn)
    ...
    def searchOpen(self, OPEN, goal_fn, heur_fn, ss, LIMIT, initState):
        ...
        while not OPEN.empty():
            ...
            for succ in successors:
                ...
                # (1-2 lines)
                prune_succ = False
                if hash_state in self.cc_dictionary and succ.gval > self.cc_dictionary[hash_state]:
...
if __name__ == '__main__':
    ...
    loc_filepath =  'locations.txt'  # Enter path to locations.tst file
